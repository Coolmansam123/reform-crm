#!/usr/bin/env python3
"""
Geocode scraped businesses (if missing coordinates) and filter by radius.

Most results from SerpAPI already have lat/lng from Google Maps.
This script:
  1. Uses pre-configured office coordinates from .env
  2. Geocodes any businesses missing coordinates (via Nominatim, free, 1 req/sec)
  3. Filters to only keep businesses within the configured radius

Input:  .tmp/scraped_businesses.json
...
def geocode_and_filter(input_path: str, office_address: str, radius_miles: int) -> list:
    """Geocode missing coordinates, then filter businesses by distance from office."""

    geolocator = Nominatim(user_agent="reform_gorilla_marketing_v1")
...
def main():
    ...
    input_path = ".tmp/scraped_businesses.json"
    ...
    if not os.path.exists(input_path):
        print(f"ERROR: {input_path} not found. Run scrape_businesses.py first.")
        return

    filtered = geocode_and_filter(input_path, office_address, radius_miles)

    output_path = ".tmp/businesses_in_radius.json"
    os.makedirs(".tmp", exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(filtered, f, indent=2, ensure_ascii=False)

    print(f"\nSaved {len(filtered)} businesses to {output_path}")
